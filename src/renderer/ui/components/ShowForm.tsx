...
export class ShowForm extends Component <ShowFormProps, ShowFormState> {
    ...
    public onNameChanged(nameString: string) {
        this.setState({name: nameString}, this.onChange)
    }
    ...
    public onTimeChanged(date: Moment | null) {
        if (date) {
            this.setState({startTime: {h: date.hours(), m: date.minutes()}}, this.onChange)
        }
    }

    public onDurationChanged(durationString: string) {
        const duration: number = durationString === "" ? 0 : Number.parseInt(durationString)
        this.setState({duration: {amount: duration, unit: "minutes"}}, this.onChange)
    }
    ...
    public render() {
        return (
            <MuiPickersUtilsProvider utils={MomentUtils}>
                ...
                <Button onClick={() => {
                    if (this.props.onDelete) this.props.onDelete(this.state.originalName, this.state)
